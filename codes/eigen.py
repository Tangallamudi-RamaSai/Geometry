...
sys.path.insert(0, '/sdcard/geometry/codes/CoordGeo') #path to my scripts
import numpy as np
...
iradius=np.linalg.norm(F3-I)
print("i_r",iradius)

#****************************Eigen vector approach to find the contact points**********************
h = A-I         # as incentre is shifted to origin ,the vertices are also shifted 
V = np.array(([1,0],[0,1]))
u = I-I         #shifting incentre towards origin 
f = -iradius*iradius
#intermediate
...
Da, P = LA.eig(sigmat)
print('Da=\n',Da,'\n','P=\n',P,'\n')
u1 = np.array(([np.sqrt(np.abs(Da[1])),np.sqrt(np.abs(Da[0]))]))
u2 = np.array(([np.sqrt(np.abs(Da[1])),-np.sqrt(np.abs(Da[0]))]))
u1 = u1.reshape(-1,1)
u2 = u2.reshape(-1,1)
#direction vectors
m1 = P@u1
...
#print(mu1,mu2)
x1 = h + mu1*m1    #these are the contact points wrt to origin considered as incentre
x2 = h + mu2*m2
Fi=x1+I       #
Ei=x2+I       # shifting back the contact points with respect to actual incentre  
print(Fi,'\n\n',Ei)
...
tri_coords = np.block([[A,B,C,D3,I,Ei,Fi]])
plt.scatter(tri_coords[0,:], tri_coords[1,:])
vert_labels = ['A','B','C','D3','I','Ei','Fi']
...
#if using termux
plt.savefig('EigenContactPts.png')
